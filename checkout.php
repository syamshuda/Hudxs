<?php
// /checkout.php (Versi Final dengan Sistem "Pilih Voucher" & Semua Fitur Lengkap)
$page_title = "Checkout";
...
if (!empty($ids_to_fetch)) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $row['promo_data'] = $promo_data_item; // Menyimpan data promo
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="metode_pembayaran" id="metodeSaldo" value="saldo" <?php if ($user_data['saldo'] <= 0) echo 'disabled'; ?>>
                            <label class="form-check-label" for="metodeSaldo">
                                Gunakan Saldo <span class="fw-bold text-success">(Tersedia: Rp <?php echo number_format($user_data['saldo'], 0, ',', '.'); ?>)</span>
                            </label>
                        </div>
                        <div class="form-check">
                 ...
                 <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-grid mt-3">
                            <button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#voucherModal" id="btn-pilih-voucher">
                                Gunakan / Pilih Voucher
                            </button>
                        </div>
                        <hr>
...
<div class="modal fade" id="voucherModal" tabindex="-1" aria-labelledby="voucherModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header"><h5 class="modal-title" id="voucherModalLabel">Pilih Voucher</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button></div>
      <div class="modal-body"><div id="voucher-list-container"><p class="text-center text-muted">Memuat voucher Anda...</p></div></div>
      <div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button><button type="button" class="btn btn-primary" id="btn-pakai-voucher">Pakai</button></div>
    </div>
  </div>
</div>
...
$(document).ready(function() {
    ...
    let saldoPembeli = <?php echo (float)($user_data['saldo'] ?? 0); ?>;
    ...
    function updateTotalDisplay() {
        ...
        let nilaiDiskonFinal = Math.round(diskonProduk + diskonOngkir);
        ...
        if (selectedVoucher) {
            ...
            $('#btn-pilih-voucher').text('Ganti Voucher');
        } else {
            $('#voucher-applied-display').hide();
            $('#btn-pilih-voucher').text('Gunakan / Pilih Voucher');
        }

        if ($('input[name="metode_pembayaran"]:checked').val() === 'saldo') {
            if (saldoPembeli < total) {
                $('#metodeTransfer').prop('checked', true);
    ...
    function updateCheckoutButtonState() {
        ...
        if (canCheckout) {
             let totalBayar = parseFloat($('#inputTotalHarga').val()) + (biayaOngkirSaatIni > 0 ? biayaOngkirSaatIni : 0);
             if (selectedVoucher && selectedVoucher.bisa_dipakai && selectedVoucher.jenis_voucher !== 'cashback') {
                 totalBayar -= parseFloat($('#inputNilaiDiskonFinal').val());
             }
             if ($('input[name="metode_pembayaran"]:checked').val() === 'saldo' && saldoPembeli < totalBayar) {
                canCheckout = false;
                buttonText = 'Saldo Tidak Cukup';
             }
        }
        ...
        $.getJSON('api_get_vouchers.php', { subtotal: subtotal, ongkir: ongkir }, function(data) {
            ...
            if (data.vouchers && data.vouchers.length > 0) {
                data.vouchers.forEach(v => {
                    ...
                    const voucherHTML = `<div class="form-check mb-3 p-3 border rounded ${disabled ? 'bg-light text-muted' : ''}"><input class="form-check-input" type="radio" name="voucherPilihan" id="voucher-${v.id}" value='${JSON.stringify(v)}' ${disabled}><label class="form-check-label w-100" for="voucher-${v.id}"><strong class="d-block">${TeksVoucher}</strong><small>Min. belanja: Rp${parseInt(v.min_pembelian).toLocaleString()}</small><small class="d-block text-danger">${v.alasan_tidak_bisa || ''}</small></label></div>`;
    ...
    $('#btn-pakai-voucher').on('click', function() {
        const selectedRadio = $('input[name="voucherPilihan"]:checked');
    ...
    $('input[name="metode_pembayaran"]').on('change', updateTotalDisplay);
